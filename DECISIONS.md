...
## ADR-027 — Gemma 4 via the Gemini API; the policy names a family, not one variant
**Status:** Accepted · **Date:** 2026-09-07 · **Supersedes part of ADR-022** · **Owner decision**
**Decision:** generation runs on **`gemma-4-26b-a4b-it`** through the Gemini API. `gemma-4-31b-it` is also permitted. `gemma4:e4b` / `google/gemma-4-E4B-it` remain permitted for a local Ollama host. Everything outside the Gemma 4 family is still refused at gateway construction — including Google's own Gemini models, which this key can reach and which are explicitly not permitted.
**Reason:** ADR-022 fixed on E4B because it was the Gemma 4 variant small enough for modest hardware. That reasoning did not survive contact with reality: **no inference provider anywhere serves E4B.** Hugging Face's router answers *"not a chat model"* because the repository's pipeline tag is `any-to-any`, and running it locally needs a GPU this project does not have. The result was a tutor that retrieved and cited correctly and then refused to write a sentence, for three weeks. The Gemini API serves two larger Gemma 4 variants on a free key. The rule was never "E4B specifically" — it was **one declared family, no silent substitution** — and that rule is intact.
**Consequence:** `gemma-4-26b-a4b-it` is the default because it is a sparse mixture of experts, ~4B active of 26B, and answers in **11–13 s** against the dense 31B's **26–78 s**. For a live demo that difference decides it.

Three things were measured against the live API, each of which would have shipped a broken demo if assumed:

- **Thinking cannot be disabled.** `thinkingConfig.thinkingBudget = 0` returns HTTP 400, *"Thinking budget is not supported for this model."* It is overhead to be budgeted for, not switched off.
- **The reasoning scratchpad is returned inside `parts`**, flagged `thought: true`, in the same list as the answer. The join every quickstart shows would have put *"\* Constraint: Exactly two sentences"* in front of a student. `GeminiProvider._answer_text` keeps only unflagged parts.
- **A small budget yields no answer at all.** Thinking runs 400–730 tokens before the answer starts, so `maxOutputTokens=80` returns one thought part, `finishReason=MAX_TOKENS`, and nothing to show — a silent empty answer, the worst failure shape. The provider raises the caller's budget to a floor of 1,500 and raises `ProviderError` rather than returning an empty answer.

**Still true:** embeddings stay on Hugging Face with `BAAI/bge-m3`, because the corpus is indexed with it and Gemini cannot reproduce those vectors. `SplitProvider` routes generation and retrieval to different providers, and that is not a fallback chain: each capability has exactly one provider, chosen because it is the only one that can do that job.
